...
# --------------------------------------------------------------------------
# Requirements:
#   pip install streamlit pandas numpy plotly prophet ruptures requests statsmodels pycountry pmdarima
#
...
#  • "EU"     – simple average across EU member states
...
CAUSE_NAME_MAP = {
    # ... (all your mappings from earlier) ...
}
...
def load_data() -> pd.DataFrame:
    def ld(ds): return load_eurostat_series(ds).rename(columns={"Region":"Country"}).dropna(subset=["Rate"])
...
def plot_joinpoints(df, c_code, cause, c_full, cause_full):
    sub = df[(df["Country"]==c_code)&(df["Cause"]==cause)].sort_values("Year")
    cps = detect_change_points(sub["Rate"])
    fig = px.line(sub, x="Year", y="Rate", title=f"{cause_full} in {c_full}")
    for cp in cps:
...
def get_arima_forecast(df_sub, periods):
    series = df_sub.sort_values("Year")["Rate"].values
    model = pm.auto_arima(series, seasonal=False, suppress_warnings=True)
    preds = model.predict(n_periods=periods)
    last = df_sub["Year"].max()
    years = np.arange(last+1, last+1+periods)
    return pd.DataFrame({"Year":years, "ARIMA":preds})
...
def get_ets_forecast(df_sub, periods):
    ...
    last = ser.index.max()
    years = np.arange(last+1, last+1+periods)
    return pd.DataFrame({"Year":years, "ETS":preds})
...
def forecast_mortality(df_sub, periods, method):
    ...
    combined = pd.merge(hist, fc[["Year","Forecast"]], on="Year", how="outer")
    fig = px.line(combined, x="Year", y=["History","Forecast"],
...
def main():
    ...
    if df_f.empty:
        ...
        st.markdown(f"### Forecast next {forecast_years} years ({method})")
        forecast_mortality(df_f, forecast_years, method)

    st.markdown("---")
    st.info("Options: classical (Prophet, ARIMA, ETS) and simple ensemble average. For advanced ML/DL, consider sktime/gluonts or LSTM-based models.")
