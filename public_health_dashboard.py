...
def main():
    ...
    if factors:
        ...
        if not available:
            st.warning("No factors available after skipping.")
        else:
            cols_needed = available + ["Mortality"]
            model_df = model_df.dropna(subset=cols_needed)
    ...
    if pivot.shape[0] < 3:
        ...
        fig_sil = px.line(
            ...
            labels={"index":"# clusters"}, title="Silhouette Score vs. # Clusters"
        )
    ...
    st.info(
        ...
        "Unavailable factors will be skipped. Clustering uses total rates only; "
        "forecasting and joinpoints per sex."
    )
