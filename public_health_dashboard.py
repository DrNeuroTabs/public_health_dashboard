...
# Define neighbors for each EU alpha-2 code
NEIGHBORS = {
...
def main():
    ...
    # ensure we dropna before sorting
    ...
    # [Mortality joinpoints, segmented fit, APC, forecasting — unchanged]

    # [Clustering section — unchanged]

    # Global Granger (unchanged from previous)

    # Neighbor map & Granger
    st.markdown("---")
    ...
    if nbr_full:
        ...
        if len(common_n) < 2:
            st.warning("Not enough neighbor data; widen year range or choose a different focal country.")
        else:
            maxlag_n = st.slider("Neighbor max lag (yrs)", 1, 5, 2, key="nbr_lag")
            alpha_n  = st.number_input("Neighbor p-value cutoff", 0.01, 0.10, 0.05, 0.01, key="nbr_alpha")
            ...
            fig_net_n.update_layout(
                title=f"Neighbor Network (p < {alpha_n})",
                xaxis=dict(visible=False), yaxis=dict(visible=False), height=600
            )
            st.plotly_chart(fig_net_n)

    st.markdown("---")
    st.info("Adjust all parameters via the sidebar and above controls.")
