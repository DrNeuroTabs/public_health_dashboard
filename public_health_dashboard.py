...
#  • Exploratory regression on health factors
#  • Data‐driven clustering + map
#
# Uses one generic loader for any Eurostat SDMX‐TSV dataset, with
# conditional filtering for mortality vs. all other tables.
# --------------------------------------------------------------------------
...
CAUSE_NAME_MAP = {
    "TOTAL":"Total",
    "A_B":"Certain infectious & parasitic diseases",
    # … (extend all your ICD‐10 mappings) …
...
FACTOR_IDS = {
    ...
    "Physical activity":        "hlth_ehis_pe9c",
    "Fruit & veg consumption":  "hlth_ehis_fv3c",
    "Smoking prevalence":       "hlth_ehis_sk1c",
    "Social support":           "hlth_ehis_ss1c",
    "Health determinants (2008)":"hlth_det_h",
...
def load_eurostat_series(dataset_id: str) -> pd.DataFrame:
    ...
    Returns DataFrame with: Region, Year, Category, Sex, Rate
    """
    url = (
    ...
    # 1) split the first col into dims
    first = raw.columns[0]
    ...
    # conditional for mortality only
    if dataset_id.startswith("hlth_cd_"):
        if "age" in dims:
            mask &= (long["age"] == "TOTAL")
        if "sex" in dims:
            mask &= (long["sex"] == "T")
    ...
    # 4) rename geo→Region, sex→Sex, other dim→Category
...
def main():
    ...
    st.markdown("---")
    st.info(
        "Use sidebar to pick filters, forecasting method, and health factors. "
        "Mortality joinpoints, segmented fits, APC, forecasts per sex; "
        "regression merges selected factors; clustering on total rates."
    )
