...
def plot_joinpoints(df: pd.DataFrame, country_code: str, cause: str, country_name: str) -> None:
    sub = df[(df["Country"] == country_code) & (df["Cause"] == cause)].sort_values("Year")
    cps = detect_change_points(sub["Rate"])
    fig = px.line(
        sub, x="Year", y="Rate",
        title=f"{cause} Mortality Rate in {country_name}"
    )
    for cp in cps:
        if 0 < cp < len(sub):
...
def main():
    ...
    causes = sorted(df[df["Country"] == country_code]["Cause"].unique())
    ...
    st.header(f"{cause} Mortality Rate in {country_full} ({year_range[0]}–{year_range[1]})")
    if df_f.empty:
        st.warning("No data available for selected filters.")
    else:
        plot_joinpoints(df_f, country_code, cause, country_full)
