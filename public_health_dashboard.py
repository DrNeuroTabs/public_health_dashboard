...
def load_eurostat_series(dataset_id: str) -> pd.DataFrame:
    ...
    if "RT" in units: unit_val = "RT"
    elif "NR" in units: unit_val = "NR"
...
def main():
    ...
    sel_countries = st.multiselect(
        "Select countries for causality (default: all)",
        country_list,
        default=country_list
    )
    maxlag = st.slider("Max lag (years)", 1, 5, 2)
    alpha = st.number_input("Significance level (p-value cutoff)", 0.01, 0.10, 0.05, 0.01)
    ...
    if len(sel_countries) >= 2:
        ...
        pivot_gc = df_c.pivot(index="Year", columns="CountryFull", values="Rate").dropna()
        ...
        fig_hm = px.imshow(
            hm,
            text_auto=".2f",
            labels=dict(x="Predictor →", y="Target ↓", color="-log₁₀(p)"),
            title="Granger Causality Heatmap (–log₁₀ p-values)"
        )
        ...
        pos = {node: (np.cos(t), np.sin(t)) for node, t in zip(sel_countries, theta)}
        edge_x, edge_y = [], []
        for src, dst in edges:
            x0, y0 = pos[src]; x1, y1 = pos[dst]
            edge_x += [x0, x1, None]; edge_y += [y0, y1, None]
        node_x = [pos[n][0] for n in sel_countries]
        node_y = [pos[n][1] for n in sel_countries]
        ...
        fig_net.add_trace(go.Scatter(
            ...
            mode="markers+text",
            marker=dict(size=20),
            text=sel_countries, textposition="bottom center",
            hoverinfo="text"
        ))
    ...
    st.markdown("---")
    st.info(
        "Select sex(es), health factors, forecasting method, clustering years, "
        "and Granger causality parameters in the sidebar and causality section."
    )
