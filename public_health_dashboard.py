...
def load_eurostat_series(dataset_id: str) -> pd.DataFrame:
    """Generic loader for any SDMX TSV Eurostat series."""
    url = (
    ...
    # filter to annual, normalized, total population
    filt = (
        (long['freq'] == 'A') &
        (long['unit'] == 'NR') &
        (long['sex'] == 'T') &
    ...
    return (
        ...
        [['Country', 'Year', 'Cause', 'Rate']]
    )

@st.cache_data
def load_data() -> pd.DataFrame:
    """Load 1994–2010 and 2011–present, then concatenate."""
...
def compute_joinpoints_and_apc(df_sub: pd.DataFrame) -> pd.DataFrame:
    ...
    for seg in segs:
        ...
        vals = rts[seg]
        slope = sm.OLS(vals, sm.add_constant(yrs[seg])).fit().params[1]
