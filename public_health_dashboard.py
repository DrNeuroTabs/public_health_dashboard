...
def main():
    ...
    st.markdown("### Cross‐country BMI vs Mortality Regression")
    years = sorted(df["Year"].unique())
    reg_year = st.select_slider("Regression year", years, value=years[-1])

    # Pull BMI for all countries in selected year
    bmi_ds = load_eurostat_series("hlth_ehis_bm1c").rename(columns={"Region":"Country"})
    bmi_sel = (
        bmi_ds[(bmi_ds["Year"]==reg_year) & (bmi_ds["Sex"]=="T")]
        .loc[:, ["Country","Rate"]]
        .rename(columns={"Rate":"BMI"})
    )

    # Pull mortality for the same year, total sex, same cause
    mort_sel = (
        df[(df["Year"]==reg_year) & (df["Sex"]=="T") & (df["Cause"]==cause_code)]
        .loc[:, ["Country","Rate"]]
        .rename(columns={"Rate":"Mortality"})
    )

    # Merge and run OLS if enough data
    reg_df = mort_sel.merge(bmi_sel, on="Country").dropna()
    if reg_df.shape[0] < 5:
        st.warning("Not enough countries with both BMI and mortality data for that year.")
    else:
        X = sm.add_constant(reg_df["BMI"])
        y = reg_df["Mortality"]
        ols_res = sm.OLS(y, X).fit()

        st.subheader(f"OLS: Mortality ~ BMI ({reg_year})")
        st.text(ols_res.summary())
        ...
        fig = px.scatter(
            ...
            text="Country",
            trendline="ols",
            labels={"BMI":"BMI (kg/m²)", "Mortality":"Standardised mortality rate"},
            title=f"Country‐level Mortality vs BMI ({reg_year})"
        )
    ...
    if pivot.shape[0] < 3:
        ...
        km = KMeans(n_clusters=best_k, random_state=0).fit(Xc)
