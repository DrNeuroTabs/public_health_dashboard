...
def load_data() -> pd.DataFrame:
    def ld(ds): 
        d = load_eurostat_series(ds).rename(columns={"Region": "Country"}).dropna(subset=["Rate"])
        return d
...
def main():
    ...
    st.markdown("---")
    st.info("Classical (Prophet, ARIMA, ETS) and Ensemble available; for ML/DL see sktime/gluonts or LSTM approaches.")
