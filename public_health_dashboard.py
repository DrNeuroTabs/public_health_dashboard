...
def main():
    ...
    df_f=df[(df["Country"]==country_code)&(df["Cause"]==cause_code)&(df["Sex"].isin(sex_codes))&(df["Year"].between(*year_range))]
    ...
    # Health‐factor regression
    st.markdown("---"); st.header("Health Factors – Exploratory Regression")
    ...
    if factors:
        ...
        for name in factors:
            ...
            dfact=dfact[(dfact["Country"]==country_code)&(dfact["Sex"].isin(sex_codes))&(dfact["Year"].between(*year_range))][["Country","Year","Sex","Rate"]].rename(columns={"Rate":name})
            model_df=model_df.merge(dfact,on=["Country","Year","Sex"],how="left")
        if skipped: st.warning(f"Skipped unavailable factors: {', '.join(skipped)}")
        ...
        if available:
            ...
            if model_df.shape[0]>=len(available)*2:
                X=sm.add_constant(model_df[available]); y=model_df["Mortality"]
                mdl=sm.OLS(y,X).fit(); st.subheader("Regression summary"); st.text(mdl.summary())
                coefs=mdl.params.drop("const")
                st.plotly_chart(px.bar(x=coefs.index,y=coefs.values,labels={"x":"Factor","y":"Coefficient"},title="Regression Coefficients"))
            else:
    ...
    if pivot.shape[0]<3:
        ...
        st.plotly_chart(px.choropleth(clust_df,locations="iso_alpha",color="Cluster",hover_name="CountryFull",locationmode="ISO-3",scope="europe",title=f"{cause_full} Clusters (k={best_k})"))

    # Global Granger causality
    st.markdown("---"); st.header("Global Granger Causality")
    ...
    if len(sel_countries)>=2:
        ...
        if len(common)>=2:
            ...
            hm=-np.log10(pvals)
            fig_hm=px.imshow(hm,text_auto=".2f",labels={"x":"Predictor →","y":"Target ↓","color":"–log₁₀(p)"},title="Global Granger Heatmap")
    ...
    st.markdown("---"); st.header("Neighbor-Based Causality")
    ...
    map_df=pd.DataFrame({"Country":[base_code]+nbr_codes,"Role":["Focal"]+["Neighbor"]*len(nbr_codes)})
    map_df["CountryFull"]=map_df["Country"].map(COUNTRY_NAME_MAP)
    map_df["iso_alpha"]=map_df["Country"].apply(alpha3_from_a2)
    st.plotly_chart(px.choropleth(map_df,locations="iso_alpha",color="Role",hover_name="CountryFull",locationmode="ISO-3",scope="europe",title="Focal & Neighbors"))
    ...
    if nbr_codes:
        ...
        if len(common_codes)>=2:
            ...
            # rename index/columns to full names for display
            pvals_n.index=common_full; pvals_n.columns=common_full
            hm_n=-np.log10(pvals_n)
            fig_hm_n=px.imshow(hm_n,text_auto=".2f",labels={"x":"Predictor →","y":"Target ↓","color":"–log₁₀(p)"},title="Neighbor Heatmap")
