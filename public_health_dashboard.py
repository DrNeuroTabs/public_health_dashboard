...
def load_data() -> pd.DataFrame:
    def ld(id_):
        x = load_eurostat_series(id_).rename(columns={"Region":"Country","Category":"Cause"})
...
def detect_change_points(ts, pen=3) -> list:
...
def main():
    ...
    if factors:
        # pick regression years separately
        ...
        if (reg_max - reg_min) < 3:
            ...
            if panel.shape[0] < len(factors)*2:
                st.warning("Not enough observations for a reliable regression.")
            else:
    ...
    # --- clustering, Granger causality, neighbor analysis unchanged ---
    # (Insert the rest of your code exactly as before here.)
