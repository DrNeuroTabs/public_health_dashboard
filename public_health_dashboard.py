...
#  • exploratory models of health factors (with graceful handling)
# --------------------------------------------------------------------------
...
# health factor series to choose from
FACTOR_IDS = {
    "BMI": "hlth_bmi",
...
def load_eurostat_series(dataset_id: str) -> pd.DataFrame:
    ...
    sub = long[mask].copy().rename(columns={
        "geo":"Region", "icd10":"Cause", "sex":"Sex"
    })
...
def load_data() -> pd.DataFrame:
    ...
    df   = df.sort_values(["Country","Cause","Sex","Year"])
...
def plot_segmented_fit_series(df_sub: pd.DataFrame, title: str):
    ...
    segs = np.split(np.arange(len(yrs)), bkps) if bkps else [np.arange(len(yrs))]
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=yrs, y=rates,
...
def load_factor_data(factor_id: str) -> pd.DataFrame:
    # may raise HTTPError if not available
...
def main():
    ...
    factors = st.multiselect("Select health factors to include in model", list(FACTOR_IDS.keys()))
    if factors:
        ...
        if model_df.shape[0] < len(factors)*2:
            ...
            X = sm.add_constant(model_df[factors])
    ...
    st.info(
        ...
        "Unavailable factors will be skipped automatically. "
        "Clustering uses total rates only; forecasting and joinpoints per sex."
    )
